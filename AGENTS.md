 ...
 **最新 main 版本**: v2.7.0（2026-09-21；分支关系见 `docs/BRANCHES.md`，代码发布与客户部署分别记录）
...
- `SNAPSHOT_BLOB_STORE`: 显式启用 Blob；KV 不可用时保存 HTML 快照，并在 Makers Cache API 被禁止时保存 8 MB 以内的静态资源；默认关闭
- `PUBLIC_HOST`: Site Acceleration 回源时使用的对外域名；保证 HTML、Canonical、资源和跳转统一使用外部域名
- `NOINDEX_HOSTS`: 禁止搜索引擎收录的公开域名列表，以英文逗号分隔；匹配域名的最终响应添加 `X-Robots-Tag: noindex, nofollow`
